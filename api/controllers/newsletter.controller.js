import Newsletter from "../models/newsletter.model.js";
import { errorHandler } from "../utils/error.js";
export const createNewsletters = async (req, res,next) => {
    

    if(!req.body.title || !req.body.year || !req.body.author || !req.body.month ) {
        return next(errorHandler(400,'Please provide the credentials'));
    }
    const slug = (req.body.title+req.body.year + req.body.month + req.body.author )
        .split('')
        .join('-')
        .toLowerCase()
        .replace(/[^a-z0-9]/g, '');
    const newNewsletters = new Newsletter({
        ...req.body,
        slug,

    });
    try{
        const savedNewsletters  = await newNewsletters.save();
        res.status(200).json(savedNewsletters);

    }
    catch(err){
        next(err);
    }

}
export const getNewsletters = async (req, res, next) => {
    try {
        const startIndex = parseInt(req.query.startIndex) || 0;
        const limit = parseInt(req.query.limit) || 9;
        const sortDirection = req.query.order === 'asc' ? 1 : -1;
        
        // Query object to filter posts based on request parameters
        const query = {
            ...(req.query.title && { userId: req.query.title }),
            ...(req.query.author && { author: req.query.author }),
            ...(req.query.month && { month: req.query.month }),
            ...(req.query.slug && { slug: req.query.slug }),
            ...(req.query.postId && { _id: req.query.postId }),
            ...(req.query.year && { year: req.query.year }),
            ...(req.query.searchTerm && {
                $or: [
                    { title: { $regex: req.query.searchTerm, $options: 'i' } },
                    { desc: { $regex: req.query.searchTerm, $options: 'i' } },
                    { month: { $regex: req.query.searchTerm, $options: 'i' } },
                    { slug: { $regex: req.query.searchTerm, $options: 'i' } },
                    { author: { $regex: req.query.searchTerm, $options: 'i' } },
                    { year: { $regex: req.query.searchTerm, $options: 'i' } },
                
                ],
            }),
        };

        const posts = await Newsletter.find(query)
            .sort({ createdAt: -1 }) // Sort posts by createdAt in descending order
            .skip(startIndex)
            .limit(limit);

        const totalNewsLetters = await Newsletter.countDocuments(query);

        const now = new Date();
        const oneMonthAgo = new Date(
            now.getFullYear(),
            now.getMonth() - 1,
            now.getDate()
        );
        const lastMonthPosts = await Newsletter.countDocuments({
            ...query,
            createdAt: { $gte: oneMonthAgo },
        });

        res.status(200).json({
            posts,
            totalNewsLetters,
            lastMonthPosts,
        });
    } catch (error) {
        next(error);
    }
};



export const deleteNewsLetters = async (req, res, next) => {
    
    try {
        await Newsletter.findByIdAndDelete(req.params.postId);
        res.status(200).json("The post has been deleted");
    } catch (error) {
        next(error);
    }
};
